...
probar_gemini.py
-----------------
Script de PRUEBA MANUAL (no forma parte del pipeline diario): genera un
episodio completo igual que `resumen_feeds.py`, pero redactando el guion con
la API gratuita de Gemini en vez de con Claude, para comparar estilo/calidad.

Se lanza solo a mano desde el workflow
`.github/workflows/test-gemini.yml` (workflow_dispatch). No escribe en
`docs/` ni en `transcripts/`: todo el resultado (mp3, transcripción,
episodes_test.json) va a --output-dir, que el workflow sube como artifact de
la propia ejecución en vez de commitear nada al repo.

Reutiliza tal cual el resto del pipeline de `resumen_feeds.py` (feeds, Fish
Audio TTS, mezcla de música de fondo, capítulos ID3); lo único que cambia es
quién redacta el guion.
"""
...
# por una versión fonética antes de pasarlos al modelo, así el guion ya los
# cita correctamente.
PRONUNCIACIONES = {
...
def fetch_recent_entries(feeds: list, hours: int, max_entries: int = 40) -> list:
    ...
    # cuáles importan de verdad la hace el modelo, no este corte por fecha.
...
def main():
    parser = argparse.ArgumentParser(
        description="PRUEBA MANUAL: genera un episodio con Gemini en vez de Claude, sin tocar docs/ ni transcripts/")
    parser.add_argument("opml_path")
    parser.add_argument("--hours", type=int, default=24)
    parser.add_argument("--output-dir", default="test_output",
                         help="Carpeta de salida de esta prueba (mp3, transcripción, episodes_test.json)")
    parser.add_argument("--base-url", default="https://example-test.invalid",
                         help="Solo se usa para componer la URL de prueba en episodes_test.json")
    ...
    episodes_path = output_dir / "episodes_test.json"
    ...
    if not folder_entries:
        print("Sin novedades en ninguna carpeta hoy. No se genera episodio de prueba.")
        return
    ...
    print(f"Generando introducción con {GEMINI_MODEL}...")
    ...
    episode_title = f"[PRUEBA GEMINI] {now.day} de {MESES_ES[now.month - 1]} — {meta.get('titular', 'Resumen del día')}"
    ...
    final_mp3 = audio_dir / f"episodio_gemini_{date_str}.mp3"
    ...
    episodes.append({
        ...
        "modelo_guion": GEMINI_MODEL,
    })
    episodes_path.write_text(json.dumps(episodes, indent=2, ensure_ascii=False), encoding="utf-8")
    print(f"\nListo. Episodio de PRUEBA generado: {final_mp3.name} ({duration_seconds/60:.1f} min)")
